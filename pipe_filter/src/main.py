...
from pipe_filter.src.utils import *
from typing import Dict
from pipe_filter.src.db import con, sql_value
...
class Command:
    ...
    def get_lists(self, list_name: str = None):
        ...
        # Select all lists
            ...
            lists = CURSOR.execute(sql).fetchall()
            con.commit()
        return [TaskList(id=list[0], name=list[1], description=list[2]) for list in lists]
...
class ViewTasksCommand(Command):
    def __init__(self):
        super().__init__()
        self.my_pipe = Pipe([self.get_lists, self.get_tasks_for_lists, self.print_super_tasks])
    ...
    def get_tasks_for_lists(self, lists: ListType[TaskList]):
        ...
        for list in lists:
            ...
            super_tasks = [Task(id=task[0], title=task[1], description=task[2], completed=task[3], list_id=task[4], parent_id=task[5]) for task in super_tasks]
            list_and_super_tasks[list.name] = [super_task.id for super_task in super_tasks]
...
class CreateListCommand(Command):
    def __init__(self):
        super().__init__()
        self.my_pipe = Pipe([self.create_list, self.confirm_list_creation, self.display_list])
    ...
    def confirm_list_creation(self, id: int):
        ...
        if created_list:
            ...
            return TaskList(id=created_list[0], name=created_list[1], description=created_list[2])
...
class CreateTaskCommand(Command):
    def __init__(self):
        super().__init__()
        self.my_pipe = Pipe([self.create_task, self.confirm_task_creation, self.display_task])
    ...
    def create_task(self, task: Task):
        ...
        if task.parent_name:
            sql = f"SELECT id FROM '{TASK_TABLE_NAME}' WHERE title = '{task.parent_name}'"
        ...
        return Task(id=created_task[0], title=created_task[1], description=created_task[2], completed=created_task[3], list_id=created_task[4], parent_id=created_task[5],
                    list_name=task.list_name, parent_name=task.parent_name)
...
class DeleteTaskCommand(Command):
    def __init__(self):
        super().__init__()
        self.my_pipe = Pipe([self.get_ids, self.delete_task, self.delete_substasks, self.confirm_task_deletion])
...
class DeleteListCommand(Command):
    def __init__(self):
        super().__init__()
        self.my_pipe = Pipe([self.get_list_id, self.delete_list, self.delete_tasks_from_list, self.confirm_list_deletion])
...
class EditTaskCommand(Command):
    ...
    def confirm_task_changes(self, new_task: Task):
        ...
        con.commit()
        edited_task = Task(id=edited_task[0], title=edited_task[1], description=edited_task[2], completed=edited_task[3], list_id=edited_task[4], parent_id=edited_task[5])
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        prog="pipe_filter", description="A simple todo list app", epilog="Thanks for using pipe_filter!"
    )
    ...
    create_list_parser.add_argument("description", type=str, help="list description", nargs="?")
    ...
    view_lists_parser.add_argument("name", help="View a specific list (optional)", nargs="?")
    ...
    create_task_parser.add_argument("--parent_name", "-pn", help="parent task name", nargs="?")
    create_task_parser.add_argument("--description", "-d", help="task description", nargs="?")
    ...
    # View tasks
    view_tasks_parser = task_sub_parsers.add_parser("view", help="View all tasks from all or a specific list")
    view_tasks_parser.add_argument("list_name", help="View a specific list (optional)", nargs="?")

    # Edit tasks
    edit_task_parser = task_sub_parsers.add_parser("edit", help="Edit a task")
    edit_task_parser.add_argument("--orig_title", "-ot", type=str, help="original task name")
    edit_task_parser.add_argument("--list_name", "-ln", type=str, help="original list name")
    edit_task_parser.add_argument("--new_title", "-nt", type=str, help="new task name", nargs="?")
    edit_task_parser.add_argument("--new_parent_name", "-npn", type=str, help="new parent task name", nargs="?")
    edit_task_parser.add_argument("--new_description", "-nd", type=str, help="new task description", nargs="?")
    ...
    if sub_parser_name == "list":
        ...
        if list_command == "create":
            CreateListCommand().execute(TaskList(name=args.name, description=args.description))
        ...
        if list_command == "create":
            CreateTaskCommand().execute(Task(title=args.title, list_name=args.list_name,
                                             parent_name=args.parent_name, description=args.description))
        elif list_command == "delete":
            DeleteTaskCommand().execute(Task(title=args.title, list_name=args.list_name))
        elif list_command == "view":
            ViewTasksCommand().execute(args.list_name)
        elif list_command == "edit":
            EditTaskCommand().execute({"orig_task": Task(title=args.orig_title, list_name=args.list_name),
                                       "new_task": Task(title=args.new_title, parent_name=args.new_parent_name,
